...
Mappings are **informative**. They do not replace the verbatim SCORECARD / `report.json` proof for any graded product under `docs/conformance/`. Prefer absolute URLs on the published docs site when sharing externally. UHBS remains an open-source evaluation framework (Apache-2.0, v4.6.0) — not an adopted industry or academic standard.
