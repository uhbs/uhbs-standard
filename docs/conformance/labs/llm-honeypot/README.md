...
This directory holds TPS YAML and inventory helpers used by `uhbs-lab` to grade the named product. Prefer the published **full** SCORECARD and `report.json` when citing UHQS. Read modules A–F with [READING-UHQS.md](../../reports/READING-UHQS.md): protocol fidelity, behavioral realism, telemetry, Safety Gate δ_C, latency, and static audit. UHBS 4.6.0 evaluation proof is informative only — isolate honeypot networks, treat LLM API keys as secrets, and wire your own telemetry shipping in real deployments. Do not invent scores without regenerating artifacts.
