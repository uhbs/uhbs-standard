...
docker build -f Dockerfile.full -t uhbs:4.6.1-full .
```

Confirm:

```bash
docker run --rm uhbs:4.6.1 --version
docker run --rm uhbs:4.6.1-full lab --list-protocols
```
...
| `uhbs:4.6.1-full` | Installs SAST tools |
