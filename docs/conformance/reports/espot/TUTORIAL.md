...
docker build -f Dockerfile.full -t uhbs:4.6.0-full .
```

Confirm:

```bash
docker run --rm uhbs:4.6.0 --version
docker run --rm uhbs:4.6.0-full lab --list-protocols
```
...
| `uhbs:4.6.0-full` | Installs SAST tools |
